...
class Trainer:
    ...
    def __init__(
        ...
        default_callbacks = DEFAULT_CALLBACKS + \
                            get_reporting_integration_callbacks(self.args.report_to)
        ...
        # deepspeed manages its own half precision
        if (args.fp16 or args.bf16) and not args.deepspeed:
    ...
    def _hp_search_setup(self, trial: Union["optuna.Trial", Dict[str, Any]]):
        ...
        if self.hp_search_backend == HPSearchBackend.OPTUNA:
            ...
            params = {k: int(v) if isinstance(v, str)
            else v for k, v in trial.assignments.items()}
    ...
    def _wrap_model(self, model, training=True):
        ...
        if self.sharded_ddp is not None:
            ...
            model = DDP(model, device_ids=[
                dist.get_local_rank()], broadcast_buffers=False)
        elif self.args.local_rank != -1:
            kwargs = {}
    ...
    def train(
        ...
        if resume_from_checkpoint is not None:
            ...
            if args.deepspeed:
                ...
                state_dict = torch.load(os.path.join(
                    resume_from_checkpoint, WEIGHTS_NAME), map_location="cpu")
        ...
        num_examples = (
            self.num_examples(
                train_dataloader) if train_dataset_is_sized else total_train_batch_size * args.max_steps
        )
        ...
        # TODO
        tr_correct_ex = torch.tensor(0.0).to(args.device)
        tr_total_ex = torch.tensor(0.0).to(args.device)
        #####
        # _total_loss_scalar is updated everytime .item() has to be called on tr_loss and stores the sum of all losses
        self._total_loss_scalar = 0.0
        # # TODO
        # self._total_correct_scalar = 0.0
        # self._total_total_scalar = 0.0
        #####
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        # TODO
                        tr_loss_step, tr_correct_step, tr_correct_total_step = self.training_step(
                            model, inputs)
                else:
                    # TODO
                    tr_loss_step, tr_correct_step, tr_correct_total_step = self.training_step(
                ...
                # TODO
                tr_correct_ex += tr_correct_step
                tr_total_ex += tr_correct_total_step
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(
                        tr_loss, model, trial, epoch, ignore_keys_for_eval, tr_correct_ex, tr_total_ex)
                else:
        ...
        if args.load_best_model_at_end and self.state.best_model_checkpoint is not None:
            ...
            if os.path.exists(best_model_path):
                if self.deepspeed:
                    ...
                    deepspeed_engine, optimizer, lr_scheduler = deepspeed_reinit(
                        self)
                    self.model = deepspeed_engine.module
                    self.model_wrapped = deepspeed_engine
                    self.deepspeed = deepspeed_engine
        ...
        # # TODO
        # self._total_correct_scalar += tr_correct_ex.item()
        # self._total_total_scalar += tr_total_ex.item()
        # train_acc = round(self._total_correct_scalar /
        #                   self._total_total_scalar, 4)
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval, tr_correct_ex, tr_total_ex):
        if self.control.should_log:
            ...
            # TODO
            tr_correct_ex_scalar = self._nested_gather(
                tr_correct_ex).sum().item()
            tr_total_ex_scalar = self._nested_gather(tr_total_ex).sum().item()
            # reset tr_loss to zero
            tr_loss -= tr_loss
            # TODO
            tr_correct_ex -= tr_correct_ex
            tr_total_ex -= tr_total_ex
            
            logs["loss"] = round(
                tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            # TODO
            logs["mlm_acc"] = round(
                tr_correct_ex_scalar / tr_total_ex_scalar,
                4,
            )
            logs["correct"] = tr_correct_ex_scalar
            logs["total"] = tr_total_ex_scalar
            logs["learning_rate"] = self._get_learning_rate()
            
            self._total_loss_scalar += tr_loss_scalar
            # # TODO
            # self._total_correct_scalar += tr_correct_ex_scalar
            # self._total_total_scalar += tr_total_ex_scalar
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        if local_rank == -1:
            ...
            torch.save(rng_states, os.path.join(
                output_dir, f"rng_state_{local_rank}.pth"))
        ...
        if os.path.isfile(os.path.join(checkpoint, OPTIMIZER_NAME)) and os.path.isfile(
            ...
            if is_torch_tpu_available():
                ...
                optimizer_state = torch.load(os.path.join(
                    checkpoint, OPTIMIZER_NAME), map_location="cpu")
                with warnings.catch_warnings(record=True) as caught_warnings:
                    lr_scheduler_state = torch.load(os.path.join(
                        checkpoint, SCHEDULER_NAME), map_location="cpu")
                reissue_pt_warnings(caught_warnings)
                ...
                if self.do_grad_scaling and os.path.isfile(os.path.join(checkpoint, SCALER_NAME)):
                    self.scaler.load_state_dict(torch.load(
                        os.path.join(checkpoint, SCALER_NAME)))
    
    def hyperparameter_search(
            self,
            hp_space: Optional[Callable[["optuna.Trial"],
                                        Dict[str, float]]] = None,
            compute_objective: Optional[Callable[[
    ...
    def autocast_smart_context_manager(self):
        ...
        if self.use_amp:
            ...
            ctx_manager = contextlib.nullcontext() if sys.version_info >= (3,
                                                                           7) else contextlib.suppress()
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.autocast_smart_context_manager():
            loss, correct, total = self.compute_loss(
                model, inputs)
        ...
        return loss.detach(), correct.detach(), total.detach()
    
    @torch.no_grad()
    def compute_mlm_acc(self, logits, labels):
        mask = labels != -100
        p = logits.argmax(-1).masked_select(mask)
        l = labels.masked_select(mask)
        correct = (l == p).sum().float()
        return correct, torch.tensor(l.numel(), device=logits.device)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        correct, total = self.compute_mlm_acc(
        ...
        return (loss, correct, total, outputs) if return_outputs else loss, correct, total
    ...
    def _sorted_checkpoints(
        ...
        glob_checkpoints = [str(x) for x in Path(
        ...
        if self.state.best_model_checkpoint is not None:
            ...
            for i in range(best_model_index, len(checkpoints_sorted) - 2):
                checkpoints_sorted[i], checkpoints_sorted[i +
                                                          1] = checkpoints_sorted[i + 1], checkpoints_sorted[i]
    ...
    def init_git_repo(self):
        ...
        if (
                not os.path.exists(os.path.join(
                    self.args.output_dir, ".gitignore"))
    ...
    def _push_from_checkpoint(self, checkpoint_folder):
        ...
        for modeling_file in modeling_files:
            if os.path.isfile(os.path.join(checkpoint_folder, modeling_file)):
                shutil.copy(os.path.join(checkpoint_folder, modeling_file),
